...
class ParserPacketHandler extends PacketHandler{
	...
	public function handleCraftingData(CraftingDataPacket $packet) : bool{
		...
		$itemTypeDictionary = $this->itemTypeDictionary;

		foreach($packet->potionTypeRecipes as $recipe){
			$recipes["potion_type"][] = new PotionTypeRecipeData(
				$this->recipeIngredientToJson(new RecipeIngredient(new NameItemDescriptor($itemTypeDictionary->fromIntId($recipe->getInputItemId()), $recipe->getInputItemMeta()), 1)),
				$this->recipeIngredientToJson(new RecipeIngredient(new NameItemDescriptor($itemTypeDictionary->fromIntId($recipe->getIngredientItemId()), $recipe->getIngredientItemMeta()), 1)),
		...
		foreach($packet->potionContainerRecipes as $recipe){
			$recipes["potion_container_change"][] = new PotionContainerChangeRecipeData(
				$itemTypeDictionary->fromIntId($recipe->getInputItemId()),
				$this->recipeIngredientToJson(new RecipeIngredient(new NameItemDescriptor($itemTypeDictionary->fromIntId($recipe->getIngredientItemId()), 0), 1)),
				$itemTypeDictionary->fromIntId($recipe->getOutputItemId()),
			);
		}
